 ...
 * a bigger disc with the count. Hover on a mouse, tap on touch, focus on a
 * keyboard all open the matching card.
 */
        ...
        if (c.items.length > 1) {
          ...
          return (
            <button
              key={c.key}
              type="button"
              aria-label={`${c.items.length} jobs here, zoom in or open the list`}
              ...
              className={cn("group absolute flex h-14 w-14 cursor-pointer items-center justify-center rounded-full transition-[transform] duration-500", isActive && "h-20 w-20")}
              style={pos}
            >
